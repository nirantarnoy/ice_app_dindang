...
import 'package:ice_app_new/pages/producttransfersuccess.dart';
...
class ProducttransfercheckoutPage extends StatefulWidget {
  static const routeName = '/producttransfercheckout';
  @override
  _ProducttransfercheckoutPageState createState() =>
      _ProducttransfercheckoutPageState();
}

class _ProducttransfercheckoutPageState
    extends State<ProducttransfercheckoutPage> {
...
// end
  ...
  Widget _buildList(List<Addprodrec> itemlist) {
    Widget orderCards;
    if (itemlist != null) {
                  ...
                  child: GestureDetector(
                    ...
                    child: Column(
                      children: <Widget>[
                        ListTile(
                          leading: Chip(
                            label: Text('${itemlist[index].product_name}'),
                          ),
                          title: Text(
  ...
  void _submitForm2(List<Addprodrec> listitems) async {
    // Provider.of<OrderData>(context, listen: false)
    ...
    bool issave = await Provider.of<ProductData>(context, listen: false)
        .addProductTransfer(listitems);
    if (issave == true) {
      String prod_rec_no = Provider.of<ProductData>(context, listen: false)
          .product_transfer_after_add;
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ProducttransferSuccessPage(),
        ),
      );
    }
  }
  ...
  void printTicketFromSunmi(
    ...
    print("print after save transfer");
  ...
  Widget _buildtotal(List<Addprodrec> order_items) {
    double total_qty = 0;
  ...
  Widget build(BuildContext context) {
    ...
    List<Addprodrec> order_items = payment_data['orderitemlist'];
                        ...
                        if (order_items.length <= 0) {
                          ...
                          return showDialog(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: Text('ต้องการบันทึกรายการใช่หรือไม่'),
                              actions: <Widget>[
                                ElevatedButton(
                                  onPressed: () {
                                    //Navigator.of(context).pop(true);
  ...
  Widget _buildorderline(List<Addprodrec> order_items) {
    // double total_qty = 0;
    // double total_amt = 0;
    Widget _items;
    if (order_items.isNotEmpty) {
