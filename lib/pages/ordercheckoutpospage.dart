...
import 'package:ice_app_new/pages/orderpossuccess.dart';
...
class OrdercheckoutPosPage extends StatefulWidget {
  static const routeName = '/ordercheckoutpos';
  @override
  _OrdercheckoutPosPageState createState() => _OrdercheckoutPosPageState();
}

class _OrdercheckoutPosPageState extends State<OrdercheckoutPosPage> {
...
// end
  ...
  void _submitForm2(String _customer_id, List<Addorder> listitems,
    ...
    print('item list is ${listitems[0].product_name}');
    showDialog(
      context: context,
    ...
    if (issave == true) {
      ...
      if (printBinded == true && order_no_pos != '') {
        ...
        print("save success but not have order");
      }
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => OrderpossuccessPage(),
        ),
      );
    }
  }
      ...
      Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            //   child: Column(
            //     children: [
            //       Expanded(child: _buildorderline(order_items)),
            //     ],
            //   ),
            // ),
  ...
  void printTicketFromSunmi2(
      String order_no, List<Addorder> order_items) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    String emp_name = prefs.getString('emp_name');
    print("print after save pos");
    screenshotController
  ...
  Widget _buildtotal(List<Addorder> order_items) {
    double total_qty = 0;
    double total_amt = 0;
    if (order_items.isNotEmpty) {
