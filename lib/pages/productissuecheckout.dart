...
import 'package:ice_app_new/pages/productissuesuccess.dart';
...
class ProductissuecheckoutPage extends StatefulWidget {
  static const routeName = '/productissuecheckout';
  @override
  _ProductissuecheckoutPageState createState() =>
      _ProductissuecheckoutPageState();
}

class _ProductissuecheckoutPageState extends State<ProductissuecheckoutPage> {
...
// end
  ...
  Widget _buildList(List<Addissuedata> itemlist) {
    Widget orderCards;
    if (itemlist != null) {
  ...
  void _submitForm2(String _route_id, List<Addissuedata> listitems) async {
    // Provider.of<OrderData>(context, listen: false)
    ...
    bool issave = await Provider.of<IssueData>(context, listen: false)
        .addProductissue(_route_id, listitems);
    if (issave == true) {
      String order_no_pos =
          Provider.of<IssueData>(context, listen: false).issueno_after_save;
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => ProductissuesuccessPage(),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<Addissuedata> order_items = payment_data['orderitemlist'];
  ...
  void printTicketFromSunmi(
      String order_no, List<Addissuedata> order_items) async {
          ...
          children: <Widget>[
            ...
            Row(
              children: <Widget>[
                Expanded(
                    child: Text(
                  'สายส่ง ${order_items[0].route_name}',
                  style: TextStyle(
                    fontSize: 10,
                    color: Colors.black,
                  ),
  ...
  Widget _buildtotal(List<Addissuedata> order_items) {
    double total_qty = 0;
  ...
  Widget _buildorderline(List<Addissuedata> order_items) {
    // double total_qty = 0;
    // double total_amt = 0;
    Widget _items;
    if (order_items.isNotEmpty) {
